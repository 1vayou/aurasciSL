...
(function () {
  ...
  function insert() {
    ...
    // 1) Hero overlay (preferred — keeps partners on screen 1)
    var hero = document.querySelector('.hero, section.hero');
    if (hero) {
      // Make sure we have a positioning context. Most hero sections in
      // the bundle already use position:relative, but set it just in
      // case so the absolute child anchors correctly.
